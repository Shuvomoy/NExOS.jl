...
function ϵProxGen(i::Int64; tolerance_type = :default)
...
function StateLargeSparseRegModel(problem::ProblemLargeSparseRegModel, settings::Settings)
    ...
    ϵProx = ϵProxGen(i; tolerance_type = :default)
...
function InitInfoLargeSparseRegModel(problem::ProblemLargeSparseRegModel, settings::Settings)

    ## constructs the initial NExOS information
    z0 = copy(problem.z0)
    μ = copy(settings.μ_max)
    γ = γ_from_μ(μ, settings)
    ϵProx = ϵProxGen(1; tolerance_type = :default)# copy(settings.ϵ_prox)
...
function update_state_lsr!(state::StateLargeSparseRegModel, init_info::InitInfoLargeSparseRegModel, problem::ProblemLargeSparseRegModel, settings::Settings)
    ...
    state.μ = μ
    state.γ = γ
    state.ϵProx = ϵProxGen(i; tolerance_type = :default) 
...
function prox_NExOS_lsr(GMat::A, hVec::V,  γ::R, zPrev::V, xPrev::V, ϵProx::R, PMat) where {R <: Real, A <: AbstractMatrix{R}, V <:  AbstractVector{R}}
    sol = xPrev
    cg!(sol, GMat, hVec + (1/γ).*zPrev; Pl = PMat, abstol = ϵProx)
...
function update_init_info!(state::StateLargeSparseRegModel, init_info::InitInfoLargeSparseRegModel, problem::ProblemLargeSparseRegModel, settings::Settings)

    init_info.μ = init_info.μ*settings.μ_mult_fact
    init_info.γ = γ_from_μ(init_info.μ, settings)
    init_info.z0 = state.z
    init_info.ϵProx = ϵProxGen(1; tolerance_type = :default)
...
function solve!(problem::ProblemLargeSparseRegModel, settings::Settings)
